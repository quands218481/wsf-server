...
import { MailService } from 'src/core/mail/service/mail.service';
...
export class RecoverService {
  ...
  async sendUserConfirmation(user: User) {
    const { code, expiration } = await this.create(user);
    return this.mailService.sendUserConfirmation(user, code, expiration);
  }
}